...
## How it works
Генерация и отправка ПДФ по почте выполняется помощью скрипта PowerShell.
...
- act.html - шаблон акта
- generate-pdf.ps1 - скрипт с функциями:
```Powershell
Generate-MyCloudPDF  # генерация одного файла ПДФ
ConvertTextTo-Number # вспомогательная функция по преобразованию суммы в текстовое выражение 
Generate-MonthlyDocs # генерация и отправка по эл.почте счетов и актов для массива клиентов
```
Структура входного JSON или PSObject:
```JSON
var json = [{
        "Id" : "",
        "EDRPOU" : "",
